...
> There is a given array with objects in itwith id and parent number, we should first open new children key in an object with empty array and if object parent number matches other object's id, that should be nested in children array of that matched object
>
    const sortMenu = (arr) => {
